...
| **Multi-dilation ConvShift symmetric** (Paper 7 → multidil_sym, Stage 10.3-sym) | Paper 7 + internal | A | **1** (short-range temporal hierarchy) | **WIN — 0.1153 dev, ties abs-best** | Axis 1 exercised by phoneme-to-syllable window |
...
| **Paper 7 — Non-Attention LLM** (arXiv:2506.01963, Jun 2025) | Parallel dilated DWConv $\{1, 2, 4, 8\}$ on input side | **1D language / autoregressive LM** — paper-reported win; ported to ASR as `multidil_sym` (Stage 10.3-sym), dev 0.1153 / test 0.1145 — **WIN, ties Stage-3 abs-best without RSE** | Extends the single-dilation DWC pattern to a multi-scale dilation set. Our multidil_sym matches the phoneme-to-syllable window at post-subsampling frame rate (40–160 ms). Transfer to Mamba-2 / LA / LION in Stages 11 / 12. |
