...
| **CB-7 v2 (P6)** | **`rwkv6_qtail_lowrank_all_convshift_multidil_symmetric_v2`** (fixed init) | **MARGINAL** — dev 0.0989 / test 0.0988, Δ vs CB-1 v2 = **+0.0027 test CER (~2σ above)**. Basically ties CB-1 v2 within MARGINAL band.  Kronecker × working multidil doesn't add orthogonally above RSE × multidil; axis-5 (Kronecker feature) absorbed or null on top of axis-1. | 0.0989 / 0.0988 |
| **LUCID×multidil v2 (P7)** | **`rwkv6_lucid_convshift_multidil_symmetric_v2`** (fixed init) | **✅ MARGINAL — new RWKV-6 ceiling.** dev 0.0929 / test 0.0921 / WER 0.2726.  Δ vs CB-1 v2 = **−0.0040 test CER (~3σ below)**. First genuine cross-axis composition on v2 baseline: axis-2 (LUCID preconditioner) × axis-1 (multi-dilation) compose productively.  Decision band: MARGINAL (0.0929 just above 0.090 BREAK threshold).  5.85 M encoder. | 0.0929 / 0.0921 |
| **LUCID×RSE×multidil v2 (P8)** | **`rwkv6_lucid_rse_convshift_multidil_symmetric_v2`** (fixed init) | **MARGINAL** — dev 0.0958 / test 0.0969 / WER 0.2887.  Δ vs P7 = **+0.0048 test CER (WORSE than P7 by ~3σ)**.  Δ vs CB-1 v2 = +0.0008 (tied within σ).  Triple composition underperforms the P7 double composition: **RSE's transition-side contribution is absorbed by LUCID's preconditioner once LUCID is active**.  RSE × LUCID saturate together; the useful two-axis composition is LUCID × multidil, not the triple. 5.92 M encoder. | 0.0958 / 0.0969 |
...
| `linear_attn_lucid` (P9) | axis-2 (preconditioner) | v2 | **0.2080** | **0.2057** | 0.5966 | 6.26 M (enc 4.35 M + 24 lucid_temperature scalars) | 30 | **DONE — MARGINAL**, `outputs/linear_attn_lucid_seed42/`. Δ test CER vs vanilla LA (0.2201): **−0.0144 (~10σ)**. Far below the pre-registered 0.15–0.18 prediction — LUCID alone on LA is a weak mechanism compared to multidil_v2 alone (P3, 0.1700) or RSE+viscosity alone (11.2b, 0.1422). LUCID's axis-2 decorrelation doesn't leverage LA's failure mode (feature-map collapse) as effectively as RSE's bounded complex decay does. |
| `linear_attn_lucid_convshift_multidil_symmetric_v2` (P10) | axis-2 × axis-1 | v2 | **0.1744** | **0.1718** | 0.4967 | 6.28 M (enc 4.36 M) | 30 | **DONE — TIED within σ with P3** (test 0.1700): Δ **+0.0018**. Cross-axis composition null on LA: LUCID's contribution saturates or is absorbed once multidil is in place.  Strikingly different from RWKV-6 (P7 LUCID×multidil was a productive composition there).  Pre-registered 0.11–0.14 prediction not met. |
