...
This corrects the LION-LIT × LUCID falsification: LUCID *does* convert on LA when given a decay-bounded backbone, and the §5 P7-style composition (LION-S × LUCID × multidil_v2) is the strongest LION-S composition. The LA LION-S × DHO single-mechanism cell sits below it (test 0.0988) and below the LION-LIT × DHO BREAK cell (0.1042); the LION-LIT × DHO × MSDC cell (0.0961) remains the lowest LA test CER on the matrix overall.
...
| `7m_linear_attn_lion_s_rse_depth_viscosity_seed42` | `linear_attn_lion_s_rse_depth_viscosity` |
...
- **2026-04-27 23:31 UTC** — `7m_linear_attn_lion_rse_depth_viscosity_seed42`
  ...
  is non-fatal).
- **2026-04-30** — `7m_linear_attn_lion_s_rse_depth_viscosity_seed42`
  landed on 4-GPU DDP (RTX PRO 6000 Blackwell, 48 GB each), commits
  `8542002` (decay_mode='s' wiring + DDP launch) and `af03f9d` (output
  artefacts).  Best dev **0.0982**, **test 0.0988**, test WER 0.2988,
  params 6.34M.  Reading: with LION-S σ-decay already supplying the
  bounded similarity prior, the complex-pole block-SO(2) transition
  adds further BREAK headroom on top of the per-head decay.  The cell
  beats the P7-style LION-S composition (0.1129) by Δ −0.014 test and
  is the lowest LA LION-S test CER on the matrix; the LION-LIT × DHO ×
  MSDC cell (0.0961) remains the lowest LA test CER overall.
  Reinforces the decay-as-prerequisite reading: RSE × LION-S converts,
  RSE × LION-LIT converts (BREAK), RSE × LION-LIT × MSDC composes.
