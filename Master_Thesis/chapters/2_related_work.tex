...
compute.
  ...
  family to interaction orders $k \ge 3$ via prefix sufficient
  statistics that keep training in
  $O(Td)$ rather than the naive $O(T^k)$. These parametrisations differ in algebraic surface but share a common function class of polynomial value lifts at order $k=2$, and saturate to a narrow performance band on tasks whose
  structural prior does not reward dense per-channel freedom. They
  are cited here as part of the wider mechanism inventory, although
  the present thesis does not lead on this axis.
