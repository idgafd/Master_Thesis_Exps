...
This chapter establishes the mathematical foundation on which the
proposed mechanisms of Chapter~\ref{ch:proposed} are built.
Section~\ref{sec:axes} introduces the axis decomposition of linear-time
recurrent expressivity that frames the entire thesis.
Section~\ref{sec:backbones} formalises the three causal linear-time
backbones used in the experimental matrix within a unified recurrence
template, and identifies the structural axis on which they differ.
Section~\ref{sec:lion} presents the parallel form used for
bidirectional adaptation, together with the decay-class taxonomy that
motivates a controlled experiment on the Linear Attention backbone.
Section~\ref{sec:primitives} introduces three mathematical primitives,
drawn from prior literature on different substrates, that serve as the
building blocks for the proposed mechanisms. Section~\ref{sec:bounds}
closes the chapter by recalling the formal complexity-class boundaries
that bracket the diagonal class within which the three proposed
mechanisms operate.
...
treated as a single-scalar property, with one architecture said to be
\textit{more expressive} than another. This thesis adopts a different
position. The expressivity of causal linear-time recurrent
architectures does not collapse to a single scalar: it decomposes
into orthogonal axes, each exercised by a different task structure,
and a mechanism that extends the function class along one axis does
not generally convert into measured gain on a task whose structural
prior does not exercise that axis. The decomposition is the
conceptual scaffold around which the rest of the thesis is organised.

Three axes are central to the thesis and are exercised by the
empirical matrix reported in Chapter~\ref{ch:experiments}. Two
additional axes are mentioned for completeness, principally to
position the closed-cell evidence that accompanies the main matrix,
but are not exercised on the matrix's primary probes.
...
phoneme-to-syllable transitions in speech, formant trajectories in
voiced segments, $n$-gram structure in language, and adjacent-pixel
structure in vision. The structural deficit is shared across the
linear-time recurrent family that Section~\ref{sec:backbones}
formalises: each token sees only its own embedding plus the running
recurrent state, and the state cannot encode short-range
neighbourhood information as efficiently as an explicit local
operator can (Section~\ref{sec:primitives:msdc}). The diagnostic
property of an axis-one task is that improvements in short-range
mixing translate into reductions of the primary metric. For the
ASR probes used in this thesis, the primary metric is character error
rate at post-subsampling sequence lengths $T \le 500$. Linear-time
architectures exhibit characteristic failure modes on axis-one tasks,
most visibly through the scaling difficulties of Vision
Mamba~\cite{zhu2024visionmamba} on image classification beyond
modest parameter counts and through the documented gap of Audio
...
for the mechanisms proposed in this thesis. The diagonal-plus-low-rank
lift of DeltaNet, DeltaProduct, and
...
crosses the formal class boundary, and is referenced here to mark
the boundary rather than exercised. Section~\ref{sec:bounds} returns
...
The closed-cell evidence reported in Chapter~\ref{ch:experiments}
contains mechanisms that target axes four and five and that activate
under stochastic gradient descent without converting into measured
gain; in the synthesis chapter they are interpreted as axis-mismatch
evidence rather than as mechanism failures.
...
predictive law: a mechanism's gain on a given task is proportional
to the residual structural deficit the architecture leaves uncovered
along the axis the mechanism targets, and is null when the mechanism
extends the function class along an axis the task does not
exercise. The remainder of the thesis is organised around this law.
Chapter~\ref{ch:proposed} introduces three proposed mechanisms, each
targeting a specific axis. Chapter~\ref{ch:experiments} reports the
cross-architecture transfer pattern on the empirical matrix, which
exhibits the law in three different signatures across the three
mechanisms.

\begin{table}[t]
\centering
\small
\caption[Axes of expressivity for causal linear-time RNNs]{Axes of
expressivity considered in this thesis, with the diagnostic property
that distinguishes each axis, the task or task-family that exercises
it, and the role each axis plays in the empirical matrix. Axes one,
two, and three are exercised by the matrix; axes four and five are
mentioned for completeness and contextualisation of closed-cell
evidence.}
\label{tab:axes}
...
Polynomial-mixer benchmarks; closed-cell only \\
\bottomrule
\end{tabularx}
\end{table}

\section{Linear-time recurrence: unified formulation and three backbones}\label{sec:backbones}
...
Within the unified template of equation~(\ref{eq:unified}), Linear
Attention corresponds to the trivial transition $A_{t} = I$, an
input-write operator $B_{t} = \phi(K_{t})$ that produces a rank-one
outer-product update with $V_{t}$, and a separable readout direction
$C_{t} = \phi(Q_{t})$ applied with a normalisation step over $z_{t}$.
Among the three backbones, Linear Attention leaves the largest
residual structural deficit: the architecture has no native primitive
that bounds the magnitude of the accumulator, no native primitive
that selectively attenuates earlier writes, and no explicit local
mixing on the input side.
...
\end{equation}
where $\mathrm{ddlerp}_{\square}(a, b) = a + (b - a) \odot \mathrm{lora}_{\square}\!\big(a + (b - a) \odot \mu_{x}\big)$
is the Finch data-dependent linear-interpolation operator and
$\mathrm{lora}_{\square}(x) = \lambda_{\square} + \tanh(x A_{\square}) B_{\square}$
is its low-rank residual core, with $\mu_{x}$ and $\lambda_{\square}$
trainable vectors and $A_{\square}, B_{\square}$ trainable low-rank
projections. The double-exponential map ensures $w_{t} \in (0, 1)^{D/h}$
without requiring an explicit clamp.

Within the unified template of equation~(\ref{eq:unified}), RWKV-6
corresponds to a transition $A_{t} = \mathrm{diag}(w_{t})$ that is
diagonal and per-token data-dependent, an input-write operator that
produces the rank-one outer product $k_{t}^{\top} v_{t}$, and a
readout that includes the static bonus $\mathrm{diag}(u)$ in addition
to the previous-state component $s_{t-1}$. The architecture's
distinctive feature, relative to Linear Attention, is that decay is
both present and per-channel data-dependent, which gives the recurrent
state a native mechanism for selectively attenuating earlier writes
...
Within the unified template of equation~(\ref{eq:unified}), Mamba-2
corresponds to a transition $A_{t} = a_{t} I$ that is structurally
restricted to the scalar-identity form, with the scalar magnitude
$a_{t}$ controlled by a single per-token data-dependent step size
$\Delta_{t}$. The input-write parameter $B_{t}$ and readout direction
$C_{t}$ are also input-dependent through low-rank projections of
$x_{t}$. The architecture's distinctive feature, relative to RWKV-6,
is that the selectivity passes through a single scalar per token
rather than a per-channel decay vector, which simplifies the
state-space duality with kernel attention at the cost of removing
per-channel decay diversity from the transition operator.
...
\section{Bidirectional adaptation: the parallel form and decay taxonomy}\label{sec:lion}
...
For the experimental matrix of this thesis, the natural parallel-form
mapping per causal backbone is as follows. RWKV-6 inherits LION-S,
since the WKV decay vector $w_{t}$ is per-channel data-dependent
through equation~(\ref{eq:rwkv6-decay}). Mamba-2 inherits LION-S,
since the scalar $a_{t}$ is per-token data-dependent through the
selective step size $\Delta_{t}$ of equation~(\ref{eq:mamba2-delta}).
Linear Attention inherits LION-LIT, since the causal accumulator has
$A_{t} = I$ and therefore no decay to inherit. The matrix
additionally reports a controlled LION-S deployment on the Linear
Attention backbone, in which a per-token sigmoid decay is introduced
that is not present in the underlying causal backbone. The motivation
for this controlled deployment is structural and is examined in the
...
but a downstream mechanism that operates on this output, such as the
value-decorrelation primitive of Section~\ref{sec:primitives:cvd},
loses its conditioning guarantee when applied to a substrate whose
accumulation is unbounded.

This observation is the structural reason why the experimental matrix
of this thesis reports both LION-LIT and LION-S on the Linear
Attention backbone. LION-LIT is the table-default instantiation per
the natural parallel-form mapping, and serves as a controlled
baseline for the bidirectional behaviour of Linear Attention without
decay. LION-S, in contrast, adds a per-token sigmoid decay that is
not present in the underlying causal backbone, and serves as a
controlled experiment for the question of whether decay is
structurally required for the value-decorrelation mechanism of
Chapter~\ref{ch:proposed} to engage productively in the bidirectional
setting. The empirical outcome of this comparison is reported in
...
attention by Duvvuri et al. They underpin, respectively, the
...
X^{\dagger}[h, w] = \mathrm{Concat}\big(X[h{-}1,\, w,\, 0{:}C/4],\; X[h{+}1,\, w,\, C/4{:}C/2],\; X[h,\, w{-}1,\, C/2{:}3C/4],\; X[h,\, w{+}1,\, 3C/4{:}C]\big),
\end{equation}
...
time. RWKV-6's per-channel decay produces a real-diagonal transition
in $(\mathbb{R}_{+})^{K}$, which acts on each state channel as
multiplicative damping. Linear Attention's pure accumulator
corresponds to the trivial case $A_{t} = I$, which sits inside the
same real-diagonal class. Each of these is a special case of a
real-diagonal transition: state channels evolve independently, with
no representation of phase. The function class cannot represent
oscillatory dynamics, which restricts its capacity to model phenomena
including formant trajectories in voiced speech, rotational
trajectories in handwriting and motion, and any other process whose
state lives on a circle rather than a half-line.

The natural extension of the real-diagonal class that admits
oscillation while remaining inside the diagonalisable matrix family
is the block-complex transition. The standard reference for the
rotation group $SO(2)$ and its embedding into matrix Lie-group theory
is~\citet[Chapter 1]{hall2015lie}. Replacing each pair of state
channels with a two-vector that is rotated by an angle $\theta$ and
scaled by a damping factor $e^{-\lambda}$ yields the per-block
transition
...
transition in $SO(2)^{K/2} \times \mathbb{R}_{+}^{K/2}$. This is the
smallest natural extension of the real-diagonal $(\mathbb{R}_{+})^{K}$
class that admits damped oscillation. Each block has eigenvalues
$r e^{\pm i \theta}$ with $r = e^{-\lambda}$, placing the transition
inside the complex-diagonal SSM class characterised formally
by~\citet{sarrof2024expressive}. The block-complex form is therefore
a structured subset of the complex-diagonal class and inherits the
formal-language properties of that class as recalled in
...
\section{Formal expressivity bounds and the diagonal-class scope}\label{sec:bounds}

The chapter closes with a recapitulation of the formal
complexity-class boundaries surveyed in the related-work chapter,
with emphasis on the function-class membership of the three
proposed mechanisms. The purpose of this section is to make
explicit the formal scope within which the empirical matrix of
Chapter~\ref{ch:experiments} operates rather than to re-establish
...
alone can simulate. RWKV-7~\cite{peng2025rwkv7} adopts the same
diagonal-plus-low-rank parameterisation in its time-mix block.
...
The formal results recalled in this section therefore function as a
scope clarifier rather than as theorems the thesis attempts to
violate. The empirical matrix of Chapter~\ref{ch:experiments}
pre-registers the prediction that no mechanism in the matrix yields
a non-Abelian state-tracking gain at any scale or sequence length,
and the probes used in the experimental matrix (LibriSpeech, Common
Voice, MQAR) are chosen deliberately so as not to require such a
gain. State-tracking probes such as $S_{n}$ permutation composition
and Dyck-$k$ for $k \ge 2$ are deferred to future work on
diagonal-plus-low-rank mechanisms; the natural extension of the
framework is to repeat the cross-architecture transfer matrix on a
diagonal-plus-low-rank backbone class with mechanisms that target
the third axis introduced in Section~\ref{sec:axes}.
