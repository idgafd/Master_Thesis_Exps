...
five mechanism cells of that configuration. The composition
column reports the test character error rate of the best
pre-registered composition (CVD~$\times$~MSDC on RWKV-6 and
Mamba-2; DHO~$\times$~MSDC on Linear Attention). Bold values mark
the matrix ceiling at the 14M Mamba-2 row. The single positive
$\Delta$ in the matrix is the LA LION-LIT~$\times$~CVD cell at
$0.3194$; this cell is examined in
...
Mamba-2 causal 14M     & 0.0827 & \textbf{0.0631} & 0.0728 & 0.0781 & \textbf{0.0618} \\
...
causal, $\Delta = -0.0681$, the largest single-mechanism gain
in the matrix, with the BREAK pattern preserved at 14M
...
$\Delta = -0.1909$, since the block-complex transition
introduces decay through its damping channel
$e^{-\lambda_{\mathrm{eff}}}$ rather than through the LION mask.
