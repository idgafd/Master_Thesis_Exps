...
mechanism-prior alignment law that the pattern supports.
...
what the encoder mechanism is meant to deliver. The synthetic
...
(architecture, mode, scale) configuration; columns report the
five mechanism cells of that configuration. Each non-vanilla
cell reports test character error rate followed by the relative
reduction over the matched vanilla baseline in parentheses.
The composition column reports the best pre-registered
composition (CVD~$\times$~MSDC on RWKV-6 and Mamba-2;
DHO~$\times$~MSDC on Linear Attention). Bold values mark the
matrix ceiling at the 14M Mamba-2 row. The single positive
$\Delta$ in the matrix is the LA LION-LIT~$\times$~CVD cell at
$0.3194$ ($+8\%$ relative); this cell is examined in
Section~\ref{sec:experiments:lion}. Relative reductions are
scaled by each row's vanilla baseline; the deficit-proportional
ordering claim of Section~\ref{sec:experiments:transfer} is
argued in absolute $\Delta$ terms because baseline differences
shift the relative ordering on some MSDC cells.}
...
The matrix populates the mechanism-prior alignment law of
...
\section{Cross-architecture transfer pattern on LibriSpeech}\label{sec:experiments:transfer}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{figures/chapter5/F1_transfer_pattern_matrix.pdf}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{figures/chapter5/F2_deficit_proportional_ordering.pdf}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{figures/chapter5/F5_cross_scale_persistence.pdf}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{figures/chapter5/F7_causal_lion_mode.pdf}
...
pilot configuration of Appendix~\ref{app:setup:cv}. The
delta-of-deltas
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{figures/chapter5/F4_libri_cv_scatter.pdf}
...
\section{Axis-2 isolation: MQAR length sweep}\label{sec:experiments:mqar}
...
Table~\ref{tab:results:mqar-cohort}.
...
the wall-clock budget; FAIL denotes accuracy at or near zero
with the loss flat near the initialisation level;
SKIP$^{\dagger}$ denotes a cell that was not run because the
same architecture FAILed at a shorter length, since adding
distractors to a non-converging configuration cannot produce
convergence; OOM denotes the cell exceeded the configured
chunk-size memory budget. The Linear Attention vanilla cell
reaches $0.5$ per-sequence accuracy at $17$ kS but does not
reach $0.9$ within the budget; the asterisk on its $T = 64$
verdict marks this case.}
...
The pre-registered composition cells of the matrix test
whether mechanism-prior alignment extends to pairs of
mechanisms applied jointly. Composition gain over the strongest
single mechanism orders with whether the second mechanism
covers a structural deficit not already addressed by the first.
Figure~\ref{fig:f6-composition} shows the pattern across the
matrix.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{figures/chapter5/F6_composition_saturation.pdf}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{figures/chapter5/F11_training_curves.pdf}
