...
The three mechanisms introduced in the remainder of the chapter
target three distinct axes. The Multi-Scale Depthwise Convolution
mechanism of Section~\ref{sec:proposed:msdc} targets axis one, the
short-range temporal hierarchy axis, on the input side of every
backbone block, and leaves the recurrent transition unchanged. The
Damped Harmonic Oscillator mechanism of
Section~\ref{sec:proposed:dho} targets the damped-oscillator
sub-structure of axis one on the transition side, replacing the
real-diagonal transition with its block-complex closure. The Chunked
Value Decorrelation mechanism of Section~\ref{sec:proposed:cvd}
targets axis two, associative memory under interference, on the
value path, applying a chunk-local key-similarity preconditioner to
the values before they enter the recurrence. The three mechanisms
are algebraically independent and can be composed; cells in the
experimental matrix exercise individual mechanisms, pairwise
compositions, and the full triple composition.
...
(MSDC). MSDC is the architecture-specific instantiation, on the
linear-time recurrent backbones of this thesis, of the input-side
multi-scale local-mixing pattern surveyed in
Section~\ref{sec:primitives:msdc}. The mechanism addresses the
...
\centering
\small
\caption[Per-architecture deployment of Multi-Scale Depthwise
Convolution]{Per-architecture deployment of Multi-Scale Depthwise
...
MSDC instantiation in this thesis combines the multi-dilation form
of~\citet{kiruluta2025breaking} with the depthwise convolution
structure shared across the family, and deploys it on three causal
...
(DHO). DHO is the architecture-specific instantiation of the
block-complex transition with Rayleigh dissipation, introduced as a
mathematical primitive in Section~\ref{sec:primitives:dho}, on the
diagonal-class backbones whose vanilla transition is real-diagonal
or trivial. The mechanism extends the function class of the
...
\centering
\small
\caption[Per-architecture deployment of the Damped Harmonic
Oscillator]{Per-architecture deployment of the Damped Harmonic
...
parallel form. The LION deployment of DHO on RWKV-6 and Mamba-2
inherits the LION-S decay class, since both backbones have a
data-dependent decay parameter that the LION mapping preserves. The
LION deployment on Linear Attention uses the controlled LION-S
...
that scales the key-similarity exponent, and $\varepsilon I$ is a
small diagonal regulariser that preserves the well-conditioning of
$P_{c}$ on the linear-time substrate. The preconditioner is the
chunk-local analogue of the LUCID-faithful preconditioner of
equation~(\ref{eq:lucid-rn}), restricted to the chunk-local
$T_{c} \times T_{c}$ block in place of the full $T \times T$ matrix
of equations~(\ref{eq:lucid-base}) and~(\ref{eq:lucid-output}). The
preconditioned values $\tilde{V}_{c}$ are passed to the unmodified
scan of the underlying backbone in place of the raw values, so the
recurrence's transition operator and readout direction are not
changed.

The per-head temperature $\tau_{h}$ is parameterised through a
softplus link, $\tau_{h} = \mathrm{softplus}(\tau_{\mathrm{raw}, h})$,
to ensure positivity. The initialisation of $\tau_{\mathrm{raw}}$
is chosen so that $\tau_{h} = 1$ at the start of training in the
RWKV-6 deployment, in the Mamba-2 deployment, and in the LION-mode
Linear Attention deployments; the causal Linear Attention deployment
uses $\tau_{\mathrm{raw}} = 0$ at initialisation, which yields
$\tau_{h} = \ln 2$. The diagonal regulariser $\varepsilon$ takes
two values across the matrix, conditional on backbone:
$\varepsilon = 10^{-4}$ on the Mamba-2 deployment, and
$\varepsilon = 10^{-6}$ on the RWKV-6 and Linear Attention
deployments. The two values reflect a single design rationale: the
SSD chunk-local Gram matrix on Mamba-2 becomes ill-conditioned at
trained temperatures around $\tau_{h} \approx 1.5$, and the larger
$\varepsilon$ value stabilises the chunk-local solve at those
temperatures, while the RWKV-6 and Linear Attention deployments
operate at better-conditioned regimes and pair the smaller
$\varepsilon$ value with an element-wise singular-fallback in the
linear solve.
...
backbone's $O(T)$ envelope at the expense of a $T_{c}^{3}$ per-chunk
per-head linear-system solve cost, amortised across the linear-time
...
rather than direct adaptation, and the chapter states the
distinction without hedging. LUCID is a softmax-attention mechanism
deployed inside a quadratic-complexity Transformer. It operates on
the full $T \times T$ softmax-attention matrix; preserves the
$O(N^{2} d)$ complexity of standard attention; targets long-context
retrieval as its primary task domain; constructs the preconditioner
of equation~(\ref{eq:lucid-base}) as a fixed function of the keys
without any learnable temperature; and combines the preconditioner
with the standard softmax weights of equation~(\ref{eq:lucid-output})
in front of the value path, where the softmax weights are essential
to the derivation of the operator as a delta-rule update in the
exponential-kernel reproducing kernel Hilbert space.

CVD operates inside the recurrence of a linear-time RNN. It never
materialises the full $T \times T$ key-similarity matrix; it uses
chunk-local $T_{c} \times T_{c}$ systems exclusively, with $T_{c}$
typically two orders of magnitude smaller than the full sequence
length; it introduces a per-head learnable temperature $\tau_{h}$
in place of LUCID's fixed $1 / \sqrt{d}$ scaling; it adds the
diagonal regulariser $\varepsilon I$, which is absent from LUCID;
and it feeds the preconditioned values $\tilde{V}_{c}$ into the
backbone's unchanged scan in place of the softmax-weighted attention
output of equation~(\ref{eq:lucid-output}). The substrate change
from softmax attention to a linear-time recurrent state is the
load-bearing structural difference; the per-head temperature and
the diagonal regulariser are required for the chunk-local solve to
remain well-conditioned on a substrate whose values do not pass
through a softmax normalisation. The shared logical move between
LUCID and CVD is the use of a key-similarity preconditioner to
decorrelate values before they enter the memory operation,
motivated by the same observation that correlated keys cause
interference at readout in any associative-memory implementation.
The relation to the rank-one Householder erasure of
DeltaNet~\cite{schlag2021deltanet, yang2024deltanet} is parallel:
both DeltaNet and CVD remove the component of each value that
aligns with earlier keys under the key-similarity geometry, but
...
CVD has three architecture-specific deployments because the chunk
structure of the substrate differs across backbones.
Table~\ref{tab:cvd-deployment} contrasts the chunking pattern, the
preconditioner source, and the location of the chunk-local solve on
each backbone. The mechanism is value-side: in every cell, it
modifies the values that enter the recurrence and leaves the
recurrence's transition operator and readout direction unchanged.

\begin{table}[t]
\centering
\small
\caption[Per-architecture deployment of Chunked Value
Decorrelation]{Per-architecture deployment of Chunked Value
Decorrelation. The mechanism operates on chunk-local
$T_{c} \times T_{c}$ blocks on every backbone. The preconditioner
source identifies the key-side tensor that populates the chunk-local
Gram matrix, and the solve location identifies where in the
per-block computation the chunk-local linear system is inverted.}
\label{tab:cvd-deployment}
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.25}
\begin{tabularx}{\linewidth}{@{}p{2.4cm} X X X@{}}
\toprule
\textbf{Architecture} & \textbf{Chunking} & \textbf{Preconditioner source} & \textbf{Solve location} \\
\midrule
RWKV-6 &
...
the more obvious $B$-correlation analog. The choice is structural:
in the SSD scalar-identity dual form of
...
the generic form and produces a preconditioner whose interpretation
matches the RWKV-6 and Linear Attention deployments.
...
for the experimental matrix on each of the three causal backbones.
This section describes the LION code path of the matrix and the
decay-class correspondence that maps each causal backbone to its
LION counterpart.

The LION code path is realised as a shared interface convention
rather than a single shared wrapper class. Each causal backbone in
the matrix carries its own LION deployment, and the three
deployments share the algebraic form of the bidirectional parallel
attention of equations~(\ref{eq:lion-mask})
and~(\ref{eq:lion-fullatt}) but are realised by distinct kernels
specialised to each backbone's existing causal computation. On
RWKV-6, the LION deployment is selected by a mode parameter on the
existing Time-Mix block, with the LION branch dispatching into a
parallel attention kernel that consumes the WKV decay $w_{t}$ as
the LION mask factor. On Mamba-2, the LION deployment is selected
by a mode parameter on the SSD block, with the LION branch
dispatching into a parallel attention kernel that consumes the
per-token scalar $a_{t}$ as the LION mask factor. On Linear
Attention~\cite{katharopoulos2020transformers}, the LION deployment
is realised by a separate encoder class with its own parallel-form
forward path, since the underlying causal recurrence has no decay
parameter and the LION mask must be supplied independently. The
three deployments present a uniform algebraic interface to the rest
of the chapter and to the experimental matrix, and the chapter
refers to them collectively as the LION wrapper, but the
implementation is three coupled per-backbone deployments rather than
...
$A_{t} = I$ and therefore no decay to inherit, which corresponds to
the $\lambda_{k} = 1$ instantiation of the LION mask.

The matrix additionally reports a controlled LION-S deployment on
the Linear Attention backbone, in which a per-token sigmoid decay
is introduced that is not present in the underlying causal
backbone. The controlled deployment serves as the cleanest available
test of the structural role of decay in the bidirectional setting:
it isolates the contribution of the per-token decay parameter from
...
the same kernel-attention substrate. The structural motivation for
the controlled deployment is recalled from
Section~\ref{sec:lion}: bounded value accumulation in the
bidirectional setting requires either an external normalisation
step or a decay-bearing mask, and a downstream value-decorrelation
mechanism such as the Chunked Value Decorrelation of
Section~\ref{sec:proposed:cvd} loses its conditioning guarantee when
applied to a substrate whose accumulation is unbounded.
Table~\ref{tab:lion-mapping} summarises the per-architecture mapping.
...
deployment on Linear Attention introduces a per-token sigmoid decay
that is not present in the underlying causal backbone, and is the
only entry in the controlled column.}
