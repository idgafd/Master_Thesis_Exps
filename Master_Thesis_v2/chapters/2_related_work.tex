...
compute.
  ...
  The empirical evaluation of this thesis rests on three benchmark
  families, chosen for the structural axis each one exercises
  rather than for aggregate-score competitiveness. The principal
  probe is automatic speech recognition on
  ...
  augmentation; short post-subsampling sequences ($T \le 500$),
  deterministic greedy CTC decoding, and a single primary metric
  (character error rate) make LibriSpeech a clean axis-1
  instrument for isolating short-range temporal structure at the
  phoneme-to-syllable scale. Common
  Voice~\cite{ardila2019commonvoice} serves as a cross-distribution
  companion on the same task and metric, with a different acoustic
  ...
  study~\cite{arora2023zoology}, the cleanest available test of
  associative-memory capacity under interference. Sweeping the
  sequence length $T \in \{64, 256, 1024\}$ produces a
  length-stratified differential that is unobtainable on a
  natural audio benchmark and that operates without the variance
  overhead of autoregressive evaluation.
