...
variance of any individual cell. The empirical CER noise floor
between matched re-runs at the codebase configuration is
approximately $\sigma \approx 0.0014$, estimated from a small set
of matched-budget reruns conducted during the calibration phase
under the same preprocessing, optimiser, and decoding pipeline.
The estimate is used only as a descriptive threshold for
categorising deltas, not as a formal hypothesis test. The chapter
applies the convention that a per-cell $\Delta$ at
$|\Delta| > 5\sigma$ is in the BREAK band, $|\Delta| \le 2\sigma$
is marginal, and intermediate values are productive but not
BREAK.
...
the pre-specified composition for each architecture
...
vanilla baseline; the deficit-driven ordering claim of
...
Figure~\ref{fig:f1-transfer-pattern}; the deficit-driven
...
\caption[Deficit-driven ordering of MSDC]{Test $\Delta$ of
...
The cohort separates into three groups by verdict pattern.
Vanilla RWKV-6, Mamba-2, and Linear Attention all FAIL or
PARTIAL at $T = 64$ already, confirming the no-recall baseline
of the diagonal class without explicit interference
cancellation. Across the cohort, adding MSDC or CVD to any of
the three backbones sends the cell to PASS at every reported
length, in one to six thousand training steps. The causal
Transformer baseline PASSes at $T = 64$ and $T = 256$ within
twelve thousand steps but \textit{FAILs at $T = 1024$}. The mechanism
...
DHO behaves differently on the same cohort. Every DHO cell
FAILs at $T = 64$ already, with the Linear Attention DHO cell
additionally exceeding the configured chunk-size memory budget
at $T = 1024$. The pattern is the axis mismatch predicted by
the criterion from Section~\ref{sec:axes}: DHO extends the
function class along axis~1, the damped-oscillator sub-axis;
MQAR exercises axis~2, associative-memory capacity under
interference. A mechanism that extends the function class
along axis~1 does not convert into measured gain on a task that
exercises axis~2, regardless of how productively the same
mechanism transfers on the matched-axis probe.
...
mechanism, and pre-specified composition test character error
...
axis is not exercised by the task. Six structural signatures
from the chapter populate the criterion.

Three signatures sit on the cross-architecture transfer row.
The MSDC mechanism transfers in deficit-driven ordering across
the family (LA $>$ RWKV-6 $>$ Mamba-2 at both scales), matching
the architecture-deficit map of
...
The present matrix provides stronger evidence for the
positive-transfer side of the criterion than for the full
converse / null side. The six signatures above all sit on the
positive-transfer half: where a mechanism's axis is exercised by
the task, gain materialises in the predicted ordering. Systematic
null-effect validation on the converse half is left to future
multi-seed work; the falsification-protocol extension recorded in
