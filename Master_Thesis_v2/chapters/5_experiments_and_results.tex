...
\paragraph{Two-directional evidence.} The six signatures above
populate the positive-transfer half of the criterion: where a
mechanism's axis is exercised by the task, gain materialises in
...
measured task gain, is populated by the engaged-null catalogue of
Appendix~\ref{app:engaged-null}: seven discovery-phase mechanisms
on the LibriSpeech causal RWKV-6 spine that engage under SGD via
parameter-mobility evidence and fail to convert into CER
reduction on the axis-1 probe, plus one saturated polynomial-lift
cluster that confirms the same reading from a different
parametrisation angle. The two halves together provide
two-directional support for the criterion within the diagonal-class
function family.
