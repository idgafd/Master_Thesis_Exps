...
CVD is not a direct adaptation of LUCID. LUCID operates inside
quadratic softmax attention, materialises a full $T \times T$
preconditioner, and applies $P^{-1} V$ under softmax weights via
the construction of equations~(\ref{eq:lucid-base})
and~(\ref{eq:lucid-output}). CVD keeps only the shared logical
move, key-similarity-based value decorrelation, but applies it to
chunk-local recurrent memory updates with a learnable per-head
temperature $\tau_{h}$ in place of LUCID's fixed $1 / \sqrt{d}$
scaling. Its relation to DeltaNet's rank-one
erasure~\cite{schlag2021deltanet, yang2024deltanet} is similarly
structural: DeltaNet performs online rank-one Householder erasure
per token, whereas CVD performs closed-form decorrelation within
each chunk under the same key-similarity geometry.
