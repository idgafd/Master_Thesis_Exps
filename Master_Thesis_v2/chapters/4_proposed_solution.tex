...
This chapter introduces the experimental matrix as the chapter's
principal artefact, states the mechanism-prior alignment criterion
the matrix is built to test, and defines the three mechanisms
whose cross-architecture transfer it measures: the Multi-Scale
Depthwise Convolution, the Damped Harmonic Oscillator, and the
Chunked Value Decorrelation. The chapter also describes the
unified LION wrapper that lifts each causal backbone to its
bidirectional parallel form and the diagnostic instrumentation
used by Chapter~\ref{ch:experiments} to attribute mechanism
behaviour at the parameter level.
...
\textit{mechanism-prior alignment}. Larger residual deficits
along the axis a mechanism targets tend to correspond to larger
absolute gains; no measured gain is expected when the axis is
not exercised by the task. The criterion is two-sided. On the positive side, a
...
\textbf{RH1 (deficit-driven ordering).} If the alignment
...
narrower distribution masks. We pre-specify Common Voice
...
The three CVD forms above, and the three MSDC and DHO forms in
Sections~\ref{sec:proposed:msdc} and~\ref{sec:proposed:dho},
illustrate a property of the mechanisms shared with all three
proposed extensions of this thesis. The mechanisms are
transferable at the functional level, not necessarily at the
tensor-implementation level. Each backbone-specific form
implements the same logical operation on the chunk structure
native to that substrate: the chunk-local key-similarity
preconditioner is the operation; the chunk structure is the
substrate; the routing into the substrate is local. The shared
operation is what the cross-architecture transfer matrix tests;
the local routing is what the per-backbone formalisation
provides.
