...
decomposes into orthogonal axes, each exercised by a different
task structure, and a mechanism that extends the function class
along one axis does not generally convert into measured gain on a
task whose structural prior does not exercise that axis. The
decomposition is the conceptual scaffold around which the rest of
...
additional axes are mentioned for completeness and for
positioning the engaged-null catalogue of
Appendix~\ref{app:engaged-null} that preceded the matrix, but are
not exercised on the matrix's primary probes.
...
Mechanisms targeting axes two through five were explored in the
discovery phase of the thesis on the LibriSpeech causal RWKV-6
spine; the engaged-but-null evidence that the criterion's full
statement predicts is collected in
Appendix~\ref{app:engaged-null}, which catalogues seven
engaged-null mechanisms and one saturated polynomial-lift cluster
that activate under stochastic gradient descent without converting
into a measured CER reduction on the axis-1 LibriSpeech probe.
...
mentioned for completeness and contextualisation of the
engaged-null catalogue of Appendix~\ref{app:engaged-null}.}
\label{tab:axes}
...
Polynomial-mixer benchmarks; engaged-null catalogue
(Appendix~\ref{app:engaged-null}) \\
\bottomrule
\end{tabularx}
\end{table}
